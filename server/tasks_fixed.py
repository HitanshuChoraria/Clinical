...
# ===========================================================================
# TASK 1 ΓÇö Easy: Eligibility Violation Screening
# ===========================================================================

TASK1_PROTOCOL_SUMMARY = """
TRIAL: ONCO-2024-301 Phase II ΓÇö Investigational drug XR-7 for metastatic colorectal cancer.
SPONSOR: NovaPharma Inc.
PHASE: II (Open-label, single-arm)

INCLUSION CRITERIA (ALL must be met):
  IC-1: Age ΓëÑ 18 and Γëñ 75 years
  IC-2: Histologically confirmed metastatic colorectal adenocarcinoma
  IC-3: ECOG Performance Status 0 or 1
  IC-4: Adequate renal function: eGFR ΓëÑ 60 mL/min/1.73m┬▓
...
def grade_task1(findings: List[Dict], rationale: str) -> Tuple[float, str]:
    ...
    - Max: 100 pts ΓåÆ normalized to [0, 1]
    """
    max_pts = 100
    pts = 0
    feedback_parts = []
    ...
    if "PT-002" in flagged_subjects:
        ...
        if _mentions_any(d, ["age", "78", "ic-1", "inclusion"]):
            pts += 20
            feedback_parts.append("Γ£ô PT-002 age violation correctly identified (+20)")
        else:
            pts += 10
            feedback_parts.append("~ PT-002 flagged but violation type unclear (+10)")
    else:
        feedback_parts.append("Γ£ù PT-002 age violation missed (0)")
    ...
    if "PT-003" in flagged_subjects:
        ...
        if found_ecog and found_egfr:
            pts += 20
            feedback_parts.append("Γ£ô PT-003 both ECOG and eGFR violations found (+20)")
        ...
        feedback_parts.append("Γ£ù PT-003 violations missed (0)")
    ...
    if "PT-004" in flagged_subjects:
        ...
        if _mentions_any(d, ["kras", "sotorasib", "prior", "ic-5", "inhibitor"]):
            pts += 20
            feedback_parts.append("Γ£ô PT-004 prior KRAS inhibitor correctly identified (+20)")
        else:
            pts += 10
            feedback_parts.append("~ PT-004 flagged but violation type unclear (+10)")
    else:
        feedback_parts.append("Γ£ù PT-004 prior KRAS inhibitor violation missed (0)")
    ...
    if "PT-005" in flagged_subjects:
        ...
        if found_qtcf and found_cyp:
            pts += 20
            feedback_parts.append("Γ£ô PT-005 both QTcF and CYP3A4 violations found (+20)")
        ...
        feedback_parts.append("Γ£ù PT-005 violations missed (0)")

    # False positive penalty: PT-001 flagged
    if "PT-001" in flagged_subjects:
        pts -= 10
        feedback_parts.append("Γ£ù PT-001 incorrectly flagged as violation (-10)")
    else:
        pts += 20
        feedback_parts.append("Γ£ô PT-001 correctly not flagged as violation (+20)")
...
# ===========================================================================
# TASK 2 ΓÇö Medium: Adverse Event Severity Misclassification
# ===========================================================================

TASK2_PROTOCOL_SUMMARY = """
TRIAL: CARD-2024-112 Phase III ΓÇö Drug BX-9 for heart failure (NYHA Class IIΓÇôIII).
...
ADVERSE EVENT CLASSIFICATION (per ICH E2A):
  Grade 1 (Mild)    ΓÇö Asymptomatic/mild symptoms; no intervention needed
  Grade 2 (Moderate)ΓÇö Minimal intervention indicated; limits instrumental ADL
  Grade 3 (Severe)  ΓÇö Medically significant but not immediately life-threatening;
                       hospitalization or prolongation of hospitalization
  Grade 4 (Life-threatening) ΓÇö Urgent intervention indicated
  Grade 5 (Fatal)   ΓÇö Death related to AE

SERIOUS ADVERSE EVENTS (SAEs) must be reported to sponsor within 24 hours if:
  - Any Grade 3ΓÇô5 event
...
TASK2_ADVERSE_EVENTS = [
    ...
    },
    {
        ...
        "site_reported_grade": 2,   # MISCLASSIFIED ΓÇö should be Grade 3 SAE + AESI
        "hospitalized": True,
        "duration_days": 3,
    ...
    },
    {
        "ae_id": "AE-004",
        "subject_id": "PT-204",
        "event": "Ventricular tachycardia ΓÇö 30-second episode, asymptomatic, resolved spontaneously",
        "site_reported_grade": 1,   # MISCLASSIFIED ΓÇö VT is Grade 3 minimum; potentially Grade 4
        "hospitalized": False,
        "duration_days": 0,
        ...
        "is_sae": True,
        "misclassified": True,
        "issue": "VT always ΓëÑ Grade 3; SAE report missing; cardiac safety signal",
    },
    {
        "ae_id": "AE-005",
        "subject_id": "PT-205",
        "event": "Acute kidney injury ΓÇö creatinine 3.2x ULN",
        "site_reported_grade": 2,   # MISCLASSIFIED ΓÇö creatinine >3x ULN = Grade 3
        "hospitalized": True,
        "duration_days": 5,
    ...
    },
    {
        "ae_id": "AE-007",
        "subject_id": "PT-207",
        "event": "Fatigue ΓÇö patient reports feeling tired but continues all daily activities",
        "site_reported_grade": 3,   # MISCLASSIFIED ΓÇö should be Grade 1 (continues ADL)
        "hospitalized": False,
        "duration_days": 7,
        "intervention": "None",
...
def grade_task2(findings: List[Dict], rationale: str) -> Tuple[float, str]:
    ...
    # AE-002: dyspnea ΓÇö Grade 2ΓåÆ3, SAE, AESI
    if ae_flagged("AE-002"):
        ...
        feedback_parts.append(f"Γ£ô AE-002 flagged {'with grade+SAE correction' if grade_correct and aesi_flag else 'partially'} (+{10 if grade_correct else 5}{'+5 AESI' if aesi_flag else ''})")
    else:
        feedback_parts.append("Γ£ù AE-002 dyspnea misclassification missed (0)")

    # AE-004: VT ΓÇö Grade 1ΓåÆ3, SAE
    if ae_flagged("AE-004"):
        ...
        feedback_parts.append(f"Γ£ô AE-004 VT flagged {'with cardiac SAE recognition' if cardiac_signal else 'partially'} (+{12 if grade_correct else 6}{'+5' if cardiac_signal else ''})")
    else:
        feedback_parts.append("Γ£ù AE-004 VT misclassification missed (0)")

    # AE-005: AKI ΓÇö Grade 2ΓåÆ3
    if ae_flagged("AE-005"):
        ...
        feedback_parts.append(f"Γ£ô AE-005 AKI flagged {'with CTCAE reference' if grade_correct else 'partially'} (+{15 if grade_correct else 8})")
    else:
        feedback_parts.append("Γ£ù AE-005 AKI misclassification missed (0)")

    # AE-007: fatigue ΓÇö Grade 3ΓåÆ1 (over-reporting)
    if ae_flagged("AE-007"):
        d = get_ae_descs("AE-007")
        grade_correct = _mentions_any(d, ["grade 1", "1", "over", "over-report", "adl", "downgrade"])
        pts += 15 if grade_correct else 8
        feedback_parts.append(f"Γ£ô AE-007 fatigue over-reporting flagged {'correctly' if grade_correct else 'partially'} (+{15 if grade_correct else 8})")
    else:
        feedback_parts.append("Γ£ù AE-007 fatigue over-reporting missed (0)")
    ...
    if _mentions_any(rationale, ["sae", "reporting", "systematic", "site training", "pattern"]):
        pts += 5
        feedback_parts.append("Γ£ô Rationale identifies systemic reporting issues (+5)")
    ...
    for ae_id in correct_ae_ids:
        if ae_flagged(ae_id):
            pts -= 8
            fp_penalty += 8
            feedback_parts.append(f"Γ£ù {ae_id} incorrectly flagged as misclassified (-8)")

    if fp_penalty == 0:
        pts += 10
        feedback_parts.append("Γ£ô No false positives on correctly classified AEs (+10)")
...
# ===========================================================================
# TASK 3 ΓÇö Hard: Comprehensive Protocol Amendment Review
# ===========================================================================

TASK3_PROTOCOL_TEXT = """
PROTOCOL: NEURO-2024-450 Phase II ΓÇö Drug NX-12 for treatment-resistant depression (TRD)
...
Secondary endpoint: Response rate (ΓëÑ50% MADRS reduction) at Week 12.

PROPOSED CHANGE (Amendment A): 
  Extend treatment from 12 ΓåÆ 24 weeks with no change to primary endpoint timing.
...
INCLUSION:
  - Age 22ΓÇô65
  - DSM-5 diagnosis of MDD, current episode resistant to ΓëÑ2 adequate antidepressant trials
  - MADRS score ΓëÑ28 at screening AND baseline (within 3 days of screening)
  - Capable of providing informed consent

EXCLUSION:
  - Active suicidal ideation with plan or intent (C-SSRS score ΓëÑ4)
...
TASK3_GROUND_TRUTH_ISSUES = {
    "amendment_c_safety": {
        "description": "Amendment C allows suicidal patients (C-SSRSΓëÑ4) with plan ΓÇö unacceptable in TRD trial without intensive monitoring infrastructure beyond 'weekly check'",
        "severity": "critical",
    ...
    "amendment_f_verbal_consent": {
        "description": "Verbal consent for OLE is insufficient per ICH E6(R2) GCP ΓÇö written consent required for all interventional trial participation",
        "severity": "critical",
    ...
    "bp_monitoring_still_insufficient": {
        "description": "Even with Amendment D, BP monitoring is missing at Weeks 1, 6, 16, 20 ΓÇö given +18 mmHg known risk, monthly monitoring minimum is needed during OLE",
        "severity": "major",
    ...
    "interim_analysis_alpha_spend": {
        "description": "Amendment E adds futility interim but no alpha-spending rule or stopping boundaries specified ΓÇö protocol gap",
        "severity": "minor",
...
def grade_task3(findings: List[Dict], rationale: str) -> Tuple[float, str]:
    ...
    if n_recommendations >= 5:
        pts += 5
        feedback_parts.append(f"Γ£ô {n_recommendations} actionable recommendations provided (+5)")
...
TASKS = {
    ...
    "protocol_amendment_review": {
        ...
        "max_steps": 5,
        "protocol_summary": "NEURO-2024-450 Phase II ΓÇö Drug NX-12 for treatment-resistant depression (TRD). 6 amendments under review.",
        "patient_records": [],
        "adverse_events": [],
